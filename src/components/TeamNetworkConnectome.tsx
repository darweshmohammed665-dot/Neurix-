...
export const TeamNetworkConnectome: React.FC<TeamNetworkConnectomeProps> = ({ onSelectMember }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-4 p-4 bg-[#0f2552]/70 border border-[#ff9f00]/30 shadow-xl backdrop-blur-md">
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto">
          <button
            ...
            className={`px-4 py-2 text-xs font-bold uppercase tracking-wider flex items-center gap-2 transition-all cursor-pointer ${
              viewMode === 'network'
                ? 'bg-[#ff9f00] text-[#081838] shadow-[0_0_20px_rgba(255,159,0,0.4)]'
                : 'bg-[#081838] text-slate-300 border border-[#ff9f00]/20 hover:border-[#ff9f00]/60'
            }`}
          >
        ...
        <div className="relative w-full lg:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 bg-[#081838] border border-[#ff9f00]/20 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-[#ff9f00]"
          />
        </div>
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold uppercase transition-all cursor-pointer ${
                  isSelected
                    ? 'bg-[#ff9f00] text-[#081838]'
                    : 'bg-[#081838] text-slate-400 border border-[#ff9f00]/15 hover:border-[#ff9f00]/40 hover:text-white'
                }`}
              >
                {div}
              </button>
      ...
      {viewMode === 'network' && (
        <div className="relative bg-[#081838]/90 border border-[#ff9f00]/30 p-6 md:p-10 shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden">
          
          {/* Ambient Background Glow & Radar Pulse */}
          <div className="absolute inset-0 bg-[radial-gradient(#ff9f00_1px,transparent_1px)] [background-size:28px_28px] opacity-10 pointer-events-none" />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] bg-[#ff9f00]/5 rounded-full blur-3xl pointer-events-none" />

          {/* Network Legend & Telemetry Status */}
          <div className="flex flex-wrap items-center justify-between pb-6 mb-8 border-b border-[#ff9f00]/20 gap-4 text-xs font-mono">
            <div className="flex items-center gap-4">
              <span className="flex items-center gap-1.5 text-white">
                <span className="w-2.5 h-2.5 rounded-full bg-[#ff9f00] shadow-[0_0_8px_#ff9f00]" />
            ...
            <div className="flex items-center gap-3 text-slate-400">
              <Activity className="w-3.5 h-3.5 text-[#ff9f00] animate-pulse" />
          ...
          <div className="flex flex-col items-center space-y-12 relative z-10">
            ...
            <div className="flex flex-col items-center">
              <motion.div
                ...
                className={`relative p-5 md:p-6 bg-gradient-to-b from-[#0f2552] to-[#081838] border-2 cursor-pointer transition-all duration-300 max-w-md w-full shadow-2xl text-center group ${
                  isNodeInPath(leader.id)
                    ? 'border-[#ff9f00] shadow-[0_0_35px_rgba(255,159,0,0.6)] ring-2 ring-[#ff9f00]'
                    : 'border-[#ff9f00] hover:shadow-[0_0_30px_rgba(255,159,0,0.4)]'
                }`}
              >
                {/* Ping Beacon */}
                <span className="absolute -top-2 -right-2 flex h-4 w-4">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#ff9f00] opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-4 w-4 bg-[#ff9f00]"></span>
                </span>

                <div className="inline-flex items-center gap-1.5 px-3 py-0.5 bg-[#ff9f00] text-[#081838] text-[10px] font-black uppercase tracking-widest mb-2">
                  <Cpu className="w-3 h-3" />
                  <span>ROOT NODE // SYSTEMS ARCHITECT & LEAD</span>
                </div>

                <h3 className="text-xl md:text-2xl font-black font-display text-white group-hover:text-[#ff9f00] transition-colors">
                  {leader.name}
                </h3>
                <p className="text-xs text-[#ff9f00] font-mono mt-0.5">{leader.role}</p>

                <div className="mt-3 pt-3 border-t border-[#ff9f00]/20 flex items-center justify-between text-[10px] text-slate-400 font-mono">
              ...
              <div className="w-0.5 h-10 bg-gradient-to-b from-[#ff9f00] via-cyan-400 to-[#ff9f00] relative overflow-hidden">
                <div className="absolute inset-0 bg-white/80 animate-[ping_1.5s_infinite]" />
              </div>
            </div>
            ...
            <div className="flex flex-col items-center">
              <motion.div
                ...
                className={`relative p-4 md:p-5 bg-[#0f2552]/90 border cursor-pointer transition-all duration-300 max-w-sm w-full shadow-xl text-center group ${
                  isNodeInPath(coordinator.id)
                    ? 'border-cyan-400 shadow-[0_0_30px_rgba(34,211,238,0.5)] ring-1 ring-cyan-400'
                    : 'border-[#ff9f00]/40 hover:border-[#ff9f00]'
                }`}
              >
                <div className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#081838] border border-cyan-400/40 text-cyan-400 text-[10px] font-bold uppercase tracking-wider mb-1.5">
                ...
                <p className="text-xs text-slate-300 font-mono">{coordinator.role}</p>
              </motion.div>
              ...
              <div className="w-full max-w-4xl relative h-10 flex items-center justify-center">
                ...
                <div className="w-full h-0.5 bg-gradient-to-r from-cyan-400 via-[#ff9f00] to-emerald-400 shadow-[0_0_10px_#ff9f00]" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
              ...
              <div className="flex flex-col items-center space-y-4 p-4 bg-[#0f2552]/40 border border-cyan-500/20 shadow-lg">
                ...
                <motion.div
                  ...
                  className={`w-full p-4 bg-[#081838] border-2 cursor-pointer transition-all duration-300 group text-center relative ${
                ...
                >
                  ...
                  <h4 className="text-base font-bold font-display text-white mt-1 group-hover:text-cyan-300 transition-colors">
                    {softwareLead.name}
                  </h4>
                  ...
                  {softwareMembers.map((member) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`p-3 bg-[#081838]/90 border cursor-pointer transition-all duration-200 flex items-center justify-between group ${
                          isHighlighted
                            ? 'border-cyan-400 bg-[#0f2552] shadow-[0_0_15px_rgba(34,211,238,0.4)]'
                            : 'border-[#ff9f00]/15 hover:border-cyan-400/60'
                        }`}
                      >
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400 font-mono">{member.role}</p>
                        </div>
                        <span className="text-[9px] px-1.5 py-0.5 bg-[#0f2552] text-cyan-400 font-mono">
                          #0{member.id}
                        </span>
                      </motion.div>
                    );
              ...
              <div className="flex flex-col items-center space-y-4 p-4 bg-[#0f2552]/40 border border-[#ff9f00]/20 shadow-lg">
                ...
                <motion.div
                  ...
                  className={`w-full p-4 bg-[#081838] border-2 cursor-pointer transition-all duration-300 group text-center relative ${
                    isNodeInPath(hardwareLead.id)
                      ? 'border-[#ff9f00] shadow-[0_0_25px_rgba(255,159,0,0.5)]'
                      : 'border-[#ff9f00]/40 hover:border-[#ff9f00]'
                  }`}
                >
                  <span className="text-[9px] px-2 py-0.5 bg-[#ff9f00]/10 text-[#ff9f00] border border-[#ff9f00]/30 uppercase font-mono font-bold">
                    HARDWARE DIVISION MANAGER
                  </span>
                  <h4 className="text-base font-bold font-display text-white mt-1 group-hover:text-[#ff9f00] transition-colors">
                    {hardwareLead.name}
                  </h4>
                  <p className="text-xs text-[#ff9f00] font-mono mt-0.5">PCB & Silicon Integration Lead</p>
                </motion.div>

                {/* Sub-node connection vertical track */}
                <div className="w-0.5 h-4 bg-[#ff9f00]/40" />
                  ...
                  {hardwareMembers.map((member) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`p-3 bg-[#081838]/90 border cursor-pointer transition-all duration-200 flex items-center justify-between group ${
                          isHighlighted
                            ? 'border-[#ff9f00] bg-[#0f2552] shadow-[0_0_15px_rgba(255,159,0,0.4)]'
                            : 'border-[#ff9f00]/15 hover:border-[#ff9f00]/60'
                        }`}
                      >
                        <div>
                          <p className="text-xs font-bold text-white group-hover:text-[#ff9f00] transition-colors">
                            {member.name}
                          </p>
                          <p className="text-[10px] text-slate-400 font-mono">{member.role}</p>
                        </div>
                        <span className="text-[9px] px-1.5 py-0.5 bg-[#0f2552] text-[#ff9f00] font-mono">
                          #0{member.id}
                        </span>
                      </motion.div>
                    );
              ...
              <div className="flex flex-col items-center space-y-4 p-4 bg-[#0f2552]/40 border border-emerald-500/20 shadow-lg">
                ...
                <div className="grid grid-cols-2 gap-2 w-full">
                  <motion.div
                    ...
                    className={`p-3 bg-[#081838] border cursor-pointer transition-all duration-300 group text-center ${
                  ...
                  >
                    ...
                    <h5 className="text-xs font-bold text-white mt-1 group-hover:text-emerald-300">
                  ...
                  {presentationMembers.map((member) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`p-3 bg-[#081838]/90 border cursor-pointer transition-all duration-200 flex items-center justify-between group ${
                          isHighlighted
                            ? 'border-emerald-400 bg-[#0f2552] shadow-[0_0_15px_rgba(52,211,153,0.4)]'
                            : 'border-[#ff9f00]/15 hover:border-emerald-400/60'
                        }`}
                      >
                        <div>
                        ...
                        <span className="text-[9px] px-1.5 py-0.5 bg-[#0f2552] text-emerald-400 font-mono">
                          #0{member.id}
                        </span>
                      </motion.div>
                    );
          ...
          {teamMembers
            ...
            .map((member) => (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                onClick={() => onSelectMember(member)}
                className="p-5 bg-[#0f2552]/60 border border-[#ff9f00]/20 hover:border-[#ff9f00] hover:shadow-[0_0_25px_rgba(255,159,0,0.25)] transition-all cursor-pointer group flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between pb-2 mb-3 border-b border-[#ff9f00]/10">
                    <span className="text-[10px] px-2 py-0.5 bg-[#081838] text-[#ff9f00] font-bold uppercase">
                      {member.team}
                    </span>
                    <span className="text-[10px] text-slate-500 font-mono">#0{member.id}</span>
                  </div>
                  <h4 className="text-base font-bold font-display text-white group-hover:text-[#ff9f00] transition-colors">
                    {member.name}
                  </h4>
                  <p className="text-xs text-slate-300 mt-1 font-mono">{member.role}</p>
                  <p className="text-xs text-slate-400 mt-2 font-sans line-clamp-2">{member.bio}</p>
                </div>

                <div className="mt-4 pt-3 border-t border-[#ff9f00]/10 flex items-center justify-between text-[11px] text-[#ff9f00]">
