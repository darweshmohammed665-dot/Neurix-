...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-dark-obsidian text-slate-300 font-sans selection:bg-phosphor/20 relative">
      ...
      <div className="w-full bg-[#081838]/95 border-b border-sky-500/20 backdrop-blur-md px-4 py-2 relative z-40 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2.5 text-xs font-mono">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-sky-300 font-bold uppercase tracking-wider text-[11px] flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
              <span>NEURIX MODULE UPLOAD:</span>
            </span>
          </div>

          {/* Visible Letter Modules */}
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
            {[
              ...
              <div key={item.char} className={`flex items-center gap-1 px-2 py-0.5 rounded border ${item.color} text-[11px]`}>
                <span className="font-black font-display text-white">{item.char}</span>
                <span className="opacity-80 hidden md:inline">: {item.name}</span>
                <span className="text-[9px] px-1 bg-white/10 rounded text-emerald-300 font-sans font-semibold ml-0.5">{item.status}</span>
              </div>
            ))}
          </div>
          ...
          <button
            ...
            className="text-[10px] uppercase font-sans font-bold px-2.5 py-1 rounded bg-sky-500/20 hover:bg-sky-500/30 text-sky-200 border border-sky-400/30 transition-all flex items-center gap-1.5 cursor-pointer hover:scale-105 active:scale-95 shrink-0"
