...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-dark-obsidian text-slate-300 font-sans selection:bg-phosphor/20 relative">
      ...
      <div className="w-full bg-[#081838]/95 border-b border-sky-500/20 backdrop-blur-md px-3 sm:px-4 py-1.5 sm:py-2 relative z-40 shadow-md overflow-x-auto no-scrollbar">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 text-xs font-mono min-w-max sm:min-w-0">
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse shrink-0" />
            <span className="text-sky-300 font-bold uppercase tracking-wider text-[10px] sm:text-[11px] flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <span>NEURIX:</span>
            </span>
          </div>

          {/* Visible Letter Modules - Single Row */}
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
            {[
              { char: 'N', name: 'Neural Kernel', status: 'ACTIVE', color: 'text-blue-400 border-blue-500/30 bg-blue-950/40' },
              { char: 'E', name: 'Embedded FPGA', status: 'LOADED', color: 'text-indigo-400 border-indigo-500/30 bg-indigo-950/40' },
              { char: 'U', name: 'Universal Bus', status: 'SYNCED', color: 'text-cyan-400 border-cyan-500/30 bg-cyan-950/40' },
              { char: 'R', name: 'Resonance DSP', status: 'ONLINE', color: 'text-amber-400 border-amber-500/30 bg-amber-950/40' },
              { char: 'I', name: 'Interface MCU', status: 'LINKED', color: 'text-teal-400 border-teal-500/30 bg-teal-950/40' },
              { char: 'X', name: 'Xtensible Sys', status: 'READY', color: 'text-emerald-400 border-emerald-500/30 bg-emerald-950/40' },
            ].map((item) => (
              <div key={item.char} className={`flex items-center gap-0.5 sm:gap-1 px-1.5 py-0.5 rounded border ${item.color} text-[10px] sm:text-[11px]`}>
                <span className="font-black font-display text-white">{item.char}</span>
                <span className="opacity-80 hidden lg:inline">: {item.name}</span>
                <span className="text-[8px] sm:text-[9px] px-1 bg-white/10 rounded text-emerald-300 font-sans font-semibold ml-0.5">{item.status}</span>
              </div>
            ))}
          </div>
          ...
          <button
            ...
            className="text-[9px] sm:text-[10px] uppercase font-sans font-bold px-2 py-1 rounded bg-sky-500/20 hover:bg-sky-500/30 text-sky-200 border border-sky-400/30 transition-all flex items-center gap-1 cursor-pointer hover:scale-105 active:scale-95 shrink-0"
            title="Replay full screen intro sequence"
          >
            <RefreshCw className="w-3 h-3 text-sky-300 shrink-0" />
        ...
        <AnimatePresence mode="wait">
          {currentPage === 'home' && (
            ...
            >
              ...
              <section className="relative pt-6 pb-12 sm:pt-16 sm:pb-24 md:pt-24 md:pb-32 overflow-hidden px-4 sm:px-6 lg:px-16 w-full bg-dark-obsidian border-b border-phosphor/10">
                ...
                <div className="max-w-4xl mx-auto relative z-10 w-full flex flex-col items-center text-center">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-3.5 items-center justify-center w-full max-w-md">
                    <button 
                      onClick={() => {
                      ...
                      className="w-full sm:w-auto bg-phosphor text-dark-obsidian px-8 py-3.5 sm:px-10 sm:py-4 rounded-none font-black text-xs uppercase tracking-widest transition-all shadow-[0_4px_15px_rgba(255,159,0,0.35)] hover:shadow-[0_8px_30px_rgba(255,159,0,0.5)] hover:-translate-y-0.5 cursor-pointer shrink-0"
                    >
                    ...
                    <button 
                      ...
                      className="w-full sm:w-auto bg-dark-charcoal border border-phosphor/20 text-phosphor hover:bg-phosphor/5 px-8 py-3.5 sm:px-10 sm:py-4 rounded-none font-black text-xs uppercase tracking-widest transition-all hover:-translate-y-0.5 cursor-pointer shrink-0"
                    >
                      Signal Diagnostics
                    </button>
                  </div>
